...
function createNode(type, id, x, y) {
  ...
  node.innerHTML = `
    <div class="node wechat-node-shape">
      ${hasInputPort ? '<div class="custom-input-port" title="Input"></div>' : ''}
      <span class="wechat-icon">${iconHTML}</span>
...
function enableDrag(node) {
  ...
  node.addEventListener('mousedown', function(e) {
    ...
    const portClasses = ['custom-input-port', 'dot-connector', 'wechat-output-port', 'n8n-node-port-suboutput'];
    if (portClasses.some(cls => e.target.classList && e.target.classList.contains(cls))) return;
    isDragging = true;
...
function getPortCenter(node, portSelector, portIndex = 0, towardPoint = null) {
  ...
  if (portSelector === '.n8n-node-port-input' && node.querySelector('.custom-input-port')) {
...
function enableWeChatOutput(node) {
  ...
  if (newPlusPort && !connections.some(c => c.from === node.getAttribute('data-node-id'))) {
    newPlusPort.addEventListener('mousedown', function(e) {
      ...
      document.addEventListener('mouseup', function removeConnecting() {
        ...
        if (!connections.some(c => c.from === node.getAttribute('data-node-id'))) {
  ...
  if (newDotConnector && connections.some(c => c.from === node.getAttribute('data-node-id'))) {
